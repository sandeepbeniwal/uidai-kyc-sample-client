		...
		try {
			...
			if (DataType.P == dataType) {
				...
				System.out.println("Proto Pid in Hex: " + Hex.encodeHexString(rawPid));
                            ...
                            rawPid = createPidXML((in.gov.uidai.authentication.uid_auth_request_data._2.Pid) pid).getBytes();
			}
			...
			if (DataType.P == dataType) {
				demoBytes = ((in.gov.uidai.authserver.protobuf.Auth.Pid) pid).getDemo().toByteArray();
                            ...
                            demoBytes = getDemoXML((in.gov.uidai.authentication.uid_auth_request_data._2.Pid) pid).getBytes();
			}
