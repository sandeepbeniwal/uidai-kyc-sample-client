...
import in.gov.uidai.authentication.uid_auth_request_data._2.Bio;
import in.gov.uidai.authentication.uid_auth_request_data._2.Bios;
...
public class PidCreator {
	...
	public static ThreadLocal<in.gov.uidai.authserver.protobuf.Auth.Pid.Builder> threadLocalPidBuilderTs = new ThreadLocal<in.gov.uidai.authserver.protobuf.Auth.Pid.Builder>();
		...
		if (data != null) {
			...
			if (StringUtils.isNotBlank(data.getStaticPin()) || StringUtils.isNotBlank(data.getDynamicPin())) {
				...
				pid.getPv().setOtp(data.getDynamicPin());
                    ...
                    pid.setTs(DatatypeFactory.newDefaultInstance().newXMLGregorianCalendar(calendar));
	...
	public static in.gov.uidai.authserver.protobuf.Auth.Pid createProtoPid( DeviceCollectedAuthData data) {
		...
		if (data != null) {
			...
			if (data.getName() != null && !data.getName().isEmpty()
				...
				if (data.getDob() != null) {
					in.gov.uidai.authserver.protobuf.Auth.Dob.Builder dob = Auth.Dob.newBuilder();
			...
			if (pv.hasOtp() || pv.hasPin()) {
				pidBuilder.setPv(pv);
                    ...
                    GregorianCalendar calendar = new GregorianCalendar();
                    pidBuilder.setTs(DatatypeFactory.newDefaultInstance().newXMLGregorianCalendar(calendar).toString());
